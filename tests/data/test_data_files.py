...
def hotspots_list(config):
    df_train = pd.read_csv(
        os.path.join(config.data.files.base, config.data.files.train)
    )
...
def test_corresponding_files_from_csv(config, hotspots_list) -> None:
    ...
    environmental_data = os.listdir(
        os.path.join(config.data.files.base, config.data.files.env_data_folder)
    )
    ...
    targets = os.listdir(
        os.path.join(config.data.files.base, config.data.files.targets_folder)
    )
...
def test_nan_refl_image_values(hotspots_list, config) -> None:
    ...
    hotspots_from_sat_files = list(
        set(hotspots_from_sat_files).intersection(hotspots_list)
    )
    ...
    np.save(
        os.path.join(config.data.files.base, "hotspots_with_nan_refl_images.npy"),
        np.array(hotspots_with_nans),
    )
...
def test_nan_visual_image_values(hotspots_list, config) -> None:
    ...
    for tif_file in tqdm(hotspots_from_sat_files):
        image_path = os.path.join(
            config.data.files.base, "images_visual", tif_file + "_visual.tif"
        )
        img = tiff.imread(image_path)
        # Check if there are any NaN values in the image
        has_nan = np.isnan(img).any()
        if np.sum(has_nan):
            hotspots_with_nans.append(tif_file)  # assert not has_nan
    np.save(
        os.path.join(config.data.files.base, "hotspots_with_nan_visual_images.npy"),
        np.array(hotspots_with_nans),
    )
    assert len(hotspots_with_nans) == 0
